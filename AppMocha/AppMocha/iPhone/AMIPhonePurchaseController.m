...
#import "AMIPhoneProductShowController.h"
...
@interface AMIPhonePurchaseController()
...
@implementation AMIPhonePurchaseController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (product.isDownload && [(AMAppDelegate *)[UIApplication sharedApplication].delegate availableString:product.filePath]) {
        AMIPhoneProductShowController *showController = [[AMIPhoneProductShowController alloc] initWithNibName:@"AMIPhoneProductShowController" bundle:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *CellIdentifier = @"AMIPhoneProductCell";
    AMIPhoneProductCell *cell = (AMIPhoneProductCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (!cell) {
        [[NSBundle mainBundle] loadNibNamed:@"AMIPhoneProductCell" owner:self options:NULL];
        cell = productCell;
